...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	public void notifyDataSetChanged() {
		...
		getViewTreeObserver().addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
			...
			public void onGlobalLayout() {
				...
				setCurrentItemSelect(currentPosition);
			}
		});
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageSelected(int position) {
			...
			setCurrentItemSelect(position);
		}

	}
	
	private void setCurrentItemSelect(int position){
		if(lastPosition != position){
			tabsContainer.getChildAt(position).setSelected(true);
			if(lastPosition != -1)
				tabsContainer.getChildAt(lastPosition).setSelected(false);
